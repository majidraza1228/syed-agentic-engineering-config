...
# ── Model configuration ───────────────────────────────────────────────────────
# Default: claude-opus-4-7  (most capable, highest cost)
#
# To change the model for ALL panes, replace every --model flag in the write
# text lines below (4 occurrences). Available Claude models:
#
#   Model          Alias     Best for
#   ─────────────────────────────────────────────────────────────────────────
#   claude-opus-4-7   opus    Architecture, hard reasoning, complex tasks
#   claude-sonnet-4-6 sonnet  Everyday coding, reviews, balanced cost/quality
#   claude-haiku-4-5  haiku   Fast lookups, simple edits, high-volume tasks
#
# Mix panes (e.g. opus for high-effort + sonnet for low-effort):
#   Edit each write text line individually — they are independent.
#
# One-off override (no file edit needed):
...
tell application "iTerm"
    ...
    tell w
        ...
        tell s1
            ...
            write text "cd " & quoted form of cwd & " && printf '\\033]1337;SetBadgeFormat=%s\\a' $(printf 'opus-4.7\\nhigh effort\\n200k ctx' | base64) && claude --model claude-opus-4-7 --effort high --name High-1"
        end tell
        tell s2
            set name to "opus-4.7 | high | 200k ctx"
